...
## In progress
- [ ] (set the current task here)

## Next up
- [ ] **`next/image` pass** — galleries/covers/headshot use plain `<img loading="lazy">`; switch to `next/image` for responsive sizing + format negotiation. — `components/{Work,Freelance,Hero,About,Lightbox}*`.
